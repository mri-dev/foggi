...
class AdminUser
{
	...
	function getMegrendelesek($arg = array()){
		...
		foreach($data as $d){
			...
			$d['coupon'] 	= false;
			$d['referer'] 	= false;
			$d['kedvezmeny'] = $d['items']['total_kedvezmeny'];
	...
	public function getOrderItems($orderID, $arg = array()){
		...
		foreach($data as $d){
			$d['oldar_each'] = $d['egysegArKedvezmeny'] + $d['egysegAr'];
			$d['oldar_sub'] = $d['oldar_each'] * $d['me'];

			$totalPrice += $d[subAr];
			$tetel 		+= $d[me];
			$tkedv += $d['egysegArKedvezmeny'] * $d['me'];
	...
	**/
	...
	function saveOrderData($orderID, $post){
		...
		if( count( $cart ) > 0) {
			foreach($cart as $d) {
				$kedv  	+= $d[egysegArKedvezmeny] * $d['me'];
				$ototal += ( ($d[ar]+$d[egysegArKedvezmeny])*$d[me]);
				$total 	+= ($d[ar]*$d[me]);

				$msg 		.= '<tr>';
					$msg 		.= '<td align="center">'.$d[me].'x</td>';
					$msg 		.= '<td><a href="'.$d[url].'">'.$d[nev].'</a></td>';
					$msg 		.= '<td align="center">'. ( ($d['egysegArKedvezmeny'] != 0) ? '<strike>'.\Helper::cashFormat($d[ar]+$d['egysegArKedvezmeny']).' Ft</strike><br>' : '' ).\Helper::cashFormat($d[ar]).' Ft</td>';
					$msg 		.= '<td align="center">'.( ($d['egysegArKedvezmeny'] != 0) ? '<strike>'.\Helper::cashFormat(($d[ar]+$d['egysegArKedvezmeny'])*$d['me']).' Ft</strike><br />' : '' ).\Helper::cashFormat(($d[ar]*$d[me])).' Ft</td>';
			...
			$msg 		.= '<tr>';
				...
				$msg 		.= '<td align="center">'.\Helper::cashFormat($ototal).' Ft</td>';
			$msg 		.= '</tr>';
			...
			$msg 		.= '<tr>';
				...
				$msg 		.= '<td align="center">'.(($kedv > 0) ? '-'.\Helper::cashFormat($kedv):0).' Ft</td>';
			$msg 		.= '</tr>';
