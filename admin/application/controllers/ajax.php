				...
				case 'termekChangeActions':
					switch($mode){
						...
						case 'cikkszam':
							$eval = $val;
							$val = ($val == '') ? 'NULL' : "'".$val."'";
							$this->db->query("UPDATE shop_termekek SET cikkszam = $val, nev = $val WHERE ID = $id");

							if($val != '') {
								$xcode = explode("-", $eval);
								$this->db->query("UPDATE shop_termekek SET raktar_articleid = '{$xcode[0]}', raktar_variantid = '{$xcode[1]}' WHERE ID = $id");
							}
